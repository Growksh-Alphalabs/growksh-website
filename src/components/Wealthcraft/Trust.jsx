...
export default function Trust() {
  ...
  return (
    <section className="py-16 bg-white relative overflow-hidden">
      ...
      <div aria-hidden="true" className="absolute -left-40 -top-20 w-96 h-96 rounded-full bg-[#ffde21]/40" style={{ filter: 'blur(80px)', transform: 'scale(1.2)' }} />
      <div aria-hidden="true" className="absolute -right-40 -bottom-24 w-[520px] h-[520px] rounded-full bg-[#ffde21]/30" style={{ filter: 'blur(120px)', transform: 'scale(1.05)' }} />
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-6 relative z-10" role="list">
          {items.map((it, idx) => (
            <article
              ...
              role="listitem"
              tabIndex={0}
              className="group bg-white border border-[#ffde21]/20 rounded-xl p-5 shadow-sm focus:outline-none focus:ring-2 focus:ring-[#ffde21]/30 hover:shadow-md"
              aria-labelledby={`trust-${idx}`}
            >
              <div className="flex items-start gap-4">
                <div className="flex items-center justify-center w-10 h-10 rounded-full bg-[#ffde21] text-black shrink-0">
